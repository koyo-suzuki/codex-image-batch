...
**1ジョブ＝1プロンプト＝1画像**です。10個の異なるプロンプトを`jobs`へ入れると、最大10件を同じウェーブで開始します。

- 1ウェーブ最大10件の画像生成・編集
- PNG/JPEG/WebPのローカル参照画像
- 10個の異なるプロンプトを1ウェーブで開始
...
1. 「プロンプトをまとめて貼る」へ、1行に1プロンプトずつ入力
2. 「行ごとにジョブを作る」を押す
3. 制作フォルダへ保存
4. Codexへ`$generate-image-batch で jobs.json を一括生成して`と送る
